...
const Talk = (props: any) => {
  const comId = useParams<any>().comId;
  const name = useParams<any>().name;
  ...
  const getCheckCom = async () => {
    await axios
      .get('/communityapi/check?userId=' + userId + '&comId=' + comId)
      .then((res) => {
  ...
  const handleJoin = async () => {
    await axios
      .post('/communityapi/join?userId=' + userId + '&comId=' + comId)
      .then((res) => {
        if (checkCom) {
          setCheckCom(false);
        } else {
  ...
  const createTalk = (e: any) => {
    e.preventDefault();
    if (postTalk.context === '') {
      return alert('文字を入力してください');
    }
    axios
      .post('/talkapi/create/', postTalk)
      .then((res) => {
              ...
              <div className="col">
                {props.group.length ? (
                  props.group.map((item: any) => {
                    return (
                      <NavLink
                        to={'/context/' + item.groupId + '/' + item.name}
                        key={item.groupId}
                        className="card"
                      >
                        {item.name}
          ...
          <div className="bg-light h-100 col pe-3">
            <Switch>
              <Route path="/context/:groupId/:name">
                <TalkContext
                  talks={props.talks}
                  getTalks={(groupId: number) => props.getTalks(groupId)}
